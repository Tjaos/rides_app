...
export class DatabaseSeederService implements OnApplicationBootstrap {
  ...
  async onApplicationBootstrap() {
    ...
    for (const driver of drivers) {
      const existingDriver = await this.driverRepository.findOneBy({ name: driver.name });
      if (!existingDriver) {
    ...
    // Seed Customers
    const customers = [
      { id: 1, name: 'Maria Júlia' },
      { id: 2, name: 'Ana Clara' },
      { id: 3, name: 'João Silva' },
    ];

    for (const customer of customers) {
      const existingCustomer = await this.customerRepository.findOneBy({ id: customer.id });
      if (!existingCustomer) {
